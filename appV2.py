...
import threading
import time
import pygame  # Added for sound playback control
...
def setup_config():
    ...
    # 选择报警音频
    def choose_sound():
        filename = filedialog.askopenfilename(title="选择报警音频文件", filetypes=[("Audio Files", "*.mp3 *.wav")])
        if filename:
            config["sound_path"] = filename
            sound_label.config(text=f"音频: {os.path.basename(filename)}")
    ...
    model_label.pack()
    tk.Button(root, text="选择报警音频", command=choose_sound).pack(pady=10)
    sound_label = tk.Label(root, text="音频: 未选择")
    sound_label.pack()
...
def main_program():
    ...
    # 初始化声音
    pygame.mixer.init()
    alarm_sound = pygame.mixer.Sound(config['sound_path'])
    ...
    stop_music_button = tk.Button(root, text="停止播放音乐", command=stop_music)
